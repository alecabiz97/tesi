...
#FEEDBACK PESATO con n=3 e k=55. Usata similarità.


#Scelta dataset
#Dataset='DukeFromMarket'
#Dataset='MarketFromDuke'
def test_FeedbackPesato_CrossDataset_Similarity(Dataset):
    ...
    for i in range(n+1):
        ...
        query=queryExpansion_withFeedback_CameraFilter(ranks_index,ranks_probability,ranks_label,gallery,gallery_cams,query,query_id,query_cams,k,False)
    ...
    f=open(Dataset + '_test_complete_FeedbackPesato_Similarity_CameraFilter.pkl','wb') 
    pickle.dump(results,f)
    f.close()
    
    f=open('Ranks-'+ Dataset +'_test_complete_FeedbackPesato_Similarity_CameraFilter.pkl','wb') 
