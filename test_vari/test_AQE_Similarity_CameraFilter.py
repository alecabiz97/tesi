...
#Test AQE con n=3 e k=5. Usata la similarità.

#Scelta dataset
#Dataset='Duke'
#Dataset='Market'
def test_AQE_Similarity(Dataset):
    if Dataset == 'Duke':
    ...
    for i in range(n+1):
        ...
        query=queryExpansion_CameraFilter(ranks_index,ranks_probability,gallery,gallery_cams,query,query_cams,k,AQE=True)
    ...
    f=open(Dataset + '_test_complete_AQE_Similarity_CameraFilter.pkl','wb') 
    pickle.dump(results,f)
    f.close()
    
    f=open('Ranks-' + Dataset + '_test_complete_AQE_Similarity_CameraFilter.pkl','wb') 
